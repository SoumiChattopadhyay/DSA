...
public class Prog18_Cycle_Detection_in_Directed_Graph_Using_DFS2_Using_Only_Visited_Array {
    ...
    private static boolean dfs(int node,int[] visited,ArrayList<ArrayList<Integer>> adjList,int V){
        visited[node]=2;
        for(int neighbor:adjList.get(node)){
            //when the node has not been visited
            if(visited[neighbor]==0){
                if(dfs(neighbor,visited, adjList, V)){
                    return true;
                }
            }
            // if the node has been previously visited
            // but it has to be visited on the same path
            else if(visited[neighbor]==2){//visited 2 means we are in the same path
                return true;
            }
